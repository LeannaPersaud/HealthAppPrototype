        ...
        <Layout page="Dos and Don'ts" tooltip='Discover what activities you should and should not do at the current stage in your recovery.'
         current={2} contrast={contrast} toggleContrast={toggleContrast}>
            <div className="row text-center m-3">
                <div className="col">
                    <button type="button" className={`btn ${stage==1 ? "success-clicked" : "btn-success"} w-100`}
                     onClick={()=>setStage(1)}>View Do's</button>
                </div>
                <div className="col">
                    <button type="button" className={`btn ${stage==2 ? "danger-clicked" : "btn-danger"} w-100`}
                     onClick={()=>setStage(2)}>View Don'ts</button>
                </div>
            </div>
            {stage==1 && (<Dos/>)}
            {stage==2 && (<Donts/>)}
